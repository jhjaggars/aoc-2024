...
def get_antinodes(a: Location, b: Location):
    rise = b.y - a.y
    run = b.x - a.x

    return (Location(b.y + rise, b.x + run), Location(a.y - rise, a.x - run))
...
if __name__ == "__main__":
    ...
    for freq, antenna_set in antennas.items():
        for a, b in permutations(list(antenna_set), 2):
            for n in get_antinodes(a, b):
                if n.in_bounds(mxx, mxy):
                    map[n.y][n.x] = "#"
                    antinodes.add(n)
