...
def dijkstra(map, start, end):
    ...
    while queue:
        score, pos, heading, path = heappop(queue)
        pos = complex(*pos)

        if score > highscore:
            return highscore, best_paths
        ...
        heappush(
            queue,
            (score + 1000, uncomplex(pos), (heading + 1) % len(dirs), path),
        )
        heappush(
            queue,
            (score + 1000, uncomplex(pos), (heading - 1) % len(dirs), path),
...
if __name__ == "__main__":
    ...
    with open(fname) as fp:
        ...
        score, paths = dijkstra(map, start, end)
        tiles = set()
        tiles.add(start)
        for p in paths:
            for c in p:
